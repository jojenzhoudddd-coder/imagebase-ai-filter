...
| ID | 用例 | 预期结果 |
|----|------|----------|
| U-01 | 删除记录后点击 Undo | 记录恢复，刷新页面数据仍在 |
| U-02 | 删除字段后点击 Undo | 字段恢复，筛选条件恢复，刷新后数据仍在 |
| U-03 | 编辑单元格后 Ctrl+Z | 单元格恢复原值，刷新后数据仍是恢复后的值 |
| U-04 | 清空多个单元格后 Undo | 所有单元格恢复原值，后端同步 |
| U-05 | 快速删除后立即 Undo（竞态） | Undo 等待删除完成后再恢复，数据一致 |

### P1 — 产品体验

| ID | 用例 | 预期结果 |
|----|------|----------|
| U-06 | 后端故障时执行 Undo | 前端回退到 undo 前状态 + toast "撤销失败，数据未能同步，请刷新页面" |
| U-07 | 单元格保存后端失败 | 乐观更新回退 + toast "保存失败，修改已回退" |
| U-08 | 删除记录后端失败 | 记录恢复 + toast "删除失败，请重试" |
| U-09 | 清除单元格后端失败 | 单元格恢复 + toast "清除失败，修改已回退" |
