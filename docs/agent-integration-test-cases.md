...
| AI-INT-P0-06 | Lark CLI 配置与授权成功 | 服务端已安装 `lark-cli`，使用测试 tenant；无需用户预填 App ID / Secret | 打开 Lark 卡片 → `Test connection` 返回 `needsConfig` 或 `needsAuth` → Agent 调 `start_lark_auth`；若返回 `phase=config`，把 URL/二维码发给用户并在完成后调 `poll_lark_auth`；若返回 `phase=auth`，把 URL/code 发给用户并在完成后调 `poll_lark_auth` → 再次测试 | 配置和登录态写入该 agent/integration sandbox；后端运行 `lark-cli auth status` 成功；卡片变为 `Healthy`；用户无需 SSH 到服务器，也不需要手填 App Secret |
