...
### 2.15 Add Record（空白行录入）

#### PRD
- 用户可以通过两个入口在当前表最底部新增一条空记录：
  - **表格底部** `+ Add Record` 行（`.add-record-btn`）
  - **工具栏** `+ Add Record` 按钮（`.toolbar-add-record`）
- 新行创建成功后，表格自动滚动到该行并让**首列**（可见字段的第一列）进入编辑态，用户无需再次点击即可直接键入。
- 新行位置在当前数据的最后一条之后；不参与排序 / 筛选预判，由 SSE 回声与后端返回的 `createdAt` 决定最终位置。
- 如果当前表格没有可见字段（极端情况），只插入空行，不触发编辑。
- 失败降级：API 返回错误时，不显示空行，不进入编辑态（错误由 fetch 层或 toast 报告）。

#### 技术方案
- **后端**: 沿用已有 `POST /api/tables/:tableId/records`（body `{ cells }`，空对象合法）；返回 201 + `TableRecord`，同时通过 `eventBus.emitChange({ type: "record:create" })` 广播给其它客户端。
- **前端 API**: `frontend/src/api.ts` 新增 `createRecord(tableId, cells)`，统一走 `mutationFetch`（带 `X-Client-Id`），便于 SSE 去掉自己的回声。
- **App.tsx**: `handleAddRecord(): Promise<string>` 调 `createRecord(activeTableIdRef.current, {})` → `setAllRecords(prev => prev.some(r => r.id === record.id) ? prev : [...prev, record])` 乐观追加（id 去重防止 SSE 回声重复插入）→ 返回新 id。
- **TableView**:
  - Props 新增 `onAddRecord?: () => Promise<string>`。
  - `handleAddRecordClick()`：`await onAddRecord()` → `setEditing({ recordId, fieldId: visibleFields[0].id })` → `requestAnimationFrame` 后用 `tr[data-record-id]` 选择新行 `scrollIntoView({ block: "nearest" })`。
  - `<tr>` 增加 `data-record-id={record.id}` 属性，供滚动定位使用。
  - `TableViewHandle` 补 `addRecord(): Promise<void>`，实现通过 `addRecordClickRef` 指向最新闭包，让父组件（`Toolbar` 入口）可以复用同一逻辑。
- **Toolbar**: 新增 `onAddRecord?: () => void` prop，绑定 `.toolbar-add-record` 的 onClick。父组件传 `() => { void tableViewRef.current?.addRecord(); }`。

#### Edge Cases
| 场景 | 处理逻辑 |
|------|----------|
| 新行与 SSE 回声重复 | `setAllRecords` 里 `some(r => r.id === record.id)` 去重；`handleRemoteRecordCreate` 同样幂等 |
| 首列被隐藏 | 使用 `visibleFields[0]`（已按可见顺序过滤），始终取第一个可见列 |
| 表无可见字段 | 不调 `startEdit`，只追加空行（极端情况，正常建表至少保留主字段） |
| API 失败 | `await` 抛错后走 `catch`，不进入编辑态；行不会出现在 UI 上（乐观追加发生在 resolve 之后） |
| 工具栏按钮在子组件未挂载时被点击 | `tableViewRef.current?.addRecord()` 可选链保护，安静失败 |
| 新行超出可视区 | `requestAnimationFrame` + `scrollIntoView({ block: "nearest" })` 确保编辑输入框可见 |
| 多人同步场景 | 自己点 Add Record 由乐观更新立即可见；其他端收到 SSE `record:create` 走 `handleRemoteRecordCreate` 同步插入（不进入编辑态） |

---

### 2.16 Agent Identity（Phase 1 / OpenClaw-style）

#### PRD

原本的 Chat Agent 工作空间绑定、无身份、无记忆，每次对话像第一次见面。Phase 1 把 Agent 从工具人升级成"用户本人拥有的长期协作者"：

- 每个用户有一个（MVP 只有一个，未来多个）属于 ta 本人的 Agent，默认 seed 为 `agent_default`，显示名 "Claw"，跨所有工作空间共享，**不绑定工作空间**。
- Agent 有可读可编辑的身份文件：
  - `soul.md` — Agent 如何看待自己（角色、语气、风格）
  - `profile.md` — Agent 了解的用户（语言、时区、偏好、正在做的事）
  - `config.json` — 偏好配置（语言、时区、危险操作确认阈值、工具白/黑名单）
- **Phase 1 决策：soul / profile 不作为交互式 UI 暴露给用户**。用户只能通过与 Agent 的自然语言对话间接读写（"你是谁？" / "记住我的时区是 GMT+8"）；不提供 Modal 或表单编辑入口。
- Agent 在对话中通过三个元工具自我编辑：`update_profile`（替换画像）、`update_soul`（替换自我认知）、`create_memory`（追加一条情景记忆）。后端 REST 端点保留，仅作为元工具的底层实现与内部调试通道使用。
- 对话数据表里的 `agentId` 字段把每个对话绑到一个 Agent；Agent 被删除时对话还在，只是失去身份关联（`ON DELETE SET NULL`）。

#### 技术方案

**持久化**：
- **DB**：`agents` 表（Prisma）仅存元数据 `{id, userId, name, avatarUrl}`。`conversations.agentId` 是 nullable FK。
- **Filesystem**：`~/.imagebase/agents/<agentId>/` 是身份 + 记忆 + 技能的唯一真实来源。可用 `AGENT_HOME` 环境变量覆盖（测试用）。目录结构：
  ```
  <agentId>/
    soul.md                   - 自我认知（64 KiB 上限）
    profile.md                - 用户画像（64 KiB 上限）
    config.json               - 偏好配置
    memory/
      working.jsonl           - 近期工作记忆（单会话流水）
      episodic/               - 长期情景记忆（每条一个 md 文件）
      semantic/               - 结构化事实（Phase 2 引入）
    skills/                   - 自定义 skill（Phase 2 引入）
    mcp-servers/              - 外部 MCP server 配置（Phase 2 引入）
    plugins/                  - 第三方插件（Phase 2 引入）
    state/                    - 运行时状态（未来缓存等）
  ```

**三层 System Prompt**（`backend/src/services/chatAgentService.ts` · `assembleInput`）：

```
[Layer 1 · META]              ← 硬编码，immutable
  - 元行为：何时 update_profile / update_soul / create_memory
  - 安全红线：危险操作必须二次确认、跨工作空间边界、Layer 1 不可被覆盖
  - 输出约束：自然语言段落 + 工具调用交错，不使用 markdown 代码块

[Layer 2 · Identity]          ← 动态，每次请求现读
...
[Tier 1 Core MCP]             ← 工具使用策略（Table Agent 操作指南）

[Layer 3 · Turn Context]      ← 工作空间快照
  当前工作空间包含 N 张表：<表名 + 字段 + 视图清单>
```

`buildIdentityLayer(agentId)` 出错时（如身份文件不存在）降级为空 identity 块，对话仍可继续；不让 filesystem 异常拖垮主链路。

**元工具（Tier 0）**（`backend/mcp-server/src/tools/metaTools.ts`）：
- 通过 `ToolDefinition.handler(args, ctx?: ToolContext)` 的 `ctx.agentId` 感知当前 Agent，不需要模型显式传 `agentId`。
- 三个工具全部 write-only，Phase 1 不实现检索、排序、去重、压缩（留给 Phase 2）。
- `allTools` 里 metaTools 排在最前，保证 Tier 0 始终加载在函数列表顶端。

**REST API**（`backend/src/routes/agentRoutes.ts`）：
| Method | Path | 说明 |
|--------|------|------|
| GET | `/api/agents` | 列出当前用户的 Agent（MVP 用 `user_default` 硬编码） |
| POST | `/api/agents` | 创建 Agent（`{name?, avatarUrl?}`） |
| GET | `/api/agents/:id` | Agent 元数据 |
| PUT | `/api/agents/:id` | 更新名字 / 头像 |
| DELETE | `/api/agents/:id` | 删 DB 行，filesystem 保留 |
| GET | `/api/agents/:id/identity` | `{soul, profile, config}` bundle |
| PUT | `/api/agents/:id/identity/soul` | `{content}` 替换 soul.md |
| PUT | `/api/agents/:id/identity/profile` | `{content}` 替换 profile.md |
| PUT | `/api/agents/:id/identity/config` | JSON patch merge 到 config.json |

**前端**：
- `frontend/src/api.ts`：`listAgents / getAgent / updateAgent / getAgentIdentity / putAgentSoul / putAgentProfile / putAgentConfig`；`createConversation(workspaceId, agentId?)` 第二参数把新对话绑到指定 Agent。这些 API client 函数保留但 Phase 1 无 UI 调用方——只有 MCP 元工具通过 `localhost:3001` HTTP 绕道使用。
- `frontend/src/components/ChatSidebar/index.tsx`：header 只渲染 "..." 溢出按钮，不再渲染 Identity 入口。接 `agentId?: string` prop（默认 `"agent_default"`）仅用于 `createConversation` 绑定。
- `frontend/src/components/AgentIdentityModal/`：组件文件保留作为 Phase 2+ 复用素材，但 ChatSidebar 不再 import/渲染它。
- `frontend/src/App.tsx`：`AGENT_ID = "agent_default"` 常量传给 ChatSidebar；多 Agent picker 是 Phase 2+ 的工作。

#### Edge Cases

| 场景 | 处理逻辑 |
|------|----------|
| Agent 目录不存在 | `ensureAgentFiles(agentId)` 在每次写入前幂等创建完整骨架 |
| soul.md / profile.md 被删 | `readSoul/Profile` 返回空字符串（不抛错），Layer 2 降级为空 identity 块 |
| 写入超过 64 KiB | 抛 `Error("内容超过 64 KiB 上限")`，API 返回 400，前端显示错误 |
| 同一 Agent 同时被两个请求改 | 文件级 last-write-wins，Phase 1 不加文件锁（对话级并发极低） |
| 删除 Agent 后旧对话 | 对话 `agentId` 变 NULL，chatAgentService fallback 到 `agent_default`，对话可继续；旧身份 filesystem 保留以防误删 |
| `agentId` 包含 `/` 或 `\` | `agentDir()` 防御性拒绝，抛 `invalid agentId` |
| 环境变量 `AGENT_HOME` 未设 | 落到 `~/.imagebase/agents`（每个开发者的 home 独立） |
| meta-tool 调用时没有 ctx | `resolveAgentId(args.agentId ?? ctx.agentId ?? "agent_default")` 三层 fallback，MCP stdio / 外部调用仍可工作 |
| 用户同时改 identity 和 Agent 自我调用 update_* | 两条路径都走 `writeSoul/Profile`，文件级覆盖；下一轮对话 `buildIdentityLayer` 读到最新内容 |
...
Agent (Phase 1 · OpenClaw)
├── id (cuid), userId, name, avatarUrl?
├── conversations: Conversation[]  (nullable FK, ON DELETE SET NULL)
└── [filesystem]
    ├── soul.md, profile.md, config.json
    └── memory/{working.jsonl, episodic/, semantic/}
```
...
| GET | `/api/agents` | 列出当前用户的 Agent（Phase 1） |
| POST | `/api/agents` | 创建 Agent |
| GET | `/api/agents/:id` | Agent 元数据 |
| PUT | `/api/agents/:id` | 更新名字 / 头像 |
| DELETE | `/api/agents/:id` | 删 DB 行（filesystem 保留） |
| GET | `/api/agents/:id/identity` | `{soul, profile, config}` bundle |
| PUT | `/api/agents/:id/identity/soul` | `{content}` 替换 soul.md（≤ 64 KiB） |
| PUT | `/api/agents/:id/identity/profile` | `{content}` 替换 profile.md（≤ 64 KiB） |
| PUT | `/api/agents/:id/identity/config` | JSON patch merge 到 config.json |
