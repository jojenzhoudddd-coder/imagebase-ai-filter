...
- 用户可以通过 ChatSidebar header 的 IdentityIcon 按钮打开 Agent Identity Modal，直接编辑 soul.md 和 profile.md 的 markdown 源文本并保存。
- Agent 自己也能在对话中通过三个元工具自我编辑：`update_profile`（替换画像）、`update_soul`（替换自我认知）、`create_memory`（追加一条情景记忆）。
...
**前端**：
- `frontend/src/api.ts`：`listAgents / getAgent / updateAgent / getAgentIdentity / putAgentSoul / putAgentProfile / putAgentConfig`；`createConversation(workspaceId, agentId?)` 第二参数把新对话绑到指定 Agent。
- `frontend/src/components/AgentIdentityModal/`：overlay + 两个 monospace textarea；只 PUT 有变化的那一侧；Esc 关闭；dirty / saving / saved / error 状态在 footer。
- `frontend/src/components/ChatSidebar/index.tsx`：header 顶部加 IdentityIcon 按钮（始终可用），`agentId?` prop（默认 `"agent_default"`）。
