...
function compareValues(
  ...
  if (SELECT_TYPES.includes(ft) && field.config?.options) {
    ...
    const idxA = opts.findIndex((o: any) => o.name === String(va));
    const idxB = opts.findIndex((o: any) => o.name === String(vb));
...
/** Get the sort order label type for a field type */
export type SortLabelType = "text" | "number" | "select";

export function getSortLabelType(fieldType: FieldType): SortLabelType {
  if (NUMERIC_TYPES.includes(fieldType) || DATE_TYPES.includes(fieldType)) return "number";
