...
function useCanvasTransform() {
  ...
  const fitToView = useCallback(
    (contentBounds: { x: number; y: number; width: number; height: number }) => {
      ...
      // Only top/left/right have 60px padding; bottom is unconstrained
      const availW = vw - pad * 2;  // left + right padding
      const availH = vh - pad;      // top padding only, bottom free
...
interface Props {
  ...
  workspaceId?: string;
  /** Called after the right-click "Make interactive" flow generates a
   *  Demo from a Taste. Caller decides whether to swap the active
   *  artifact within the same MagicCanvas block (ArtifactBlock.handleSelectItem)
