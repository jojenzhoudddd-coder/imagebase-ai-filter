...
frontend/
  src/
    ...
    components/
      ...
      IdeaEditor/           - Markdown doc artifact. Dual-mode editor: Source mode (CodeMirror with markdown syntax highlighting, line wrapping, Tab indent) + Preview mode (Tiptap/ProseMirror WYSIWYG, Enter=hardBreak for 1:1 newline mapping). Cmd/Ctrl+/ toggles modes with cursor preservation. 60px padding, debounced autosave (600ms), image/SVG upload (paste/drop/toolbar, 10MB SVG limit). Key files: `index.tsx` (orchestrator ~320 lines), `CodeMirrorSource.tsx` (source mode), `TiptapPreview.tsx` (preview mode), `markdownBridge.ts` (serialization), `extensions/ImageExtension.ts` (paste/drop upload). Old `MarkdownPreview.tsx` (2268 lines) is dead code — replaced by TiptapPreview.
