...
backend/
  src/
    ...
    routes/
      ...
      agentRoutes.ts  - /api/agents + identity (soul/profile/config) + Phase 4 runtime state (inbox list/ack, cron CRUD, heartbeat tail) + multi-model endpoints (`GET /models` builtin+custom merged list, `GET|PUT /:agentId/model` selected-model) + Agent Homepage data: `GET /:agentId/memories` (episodic+working), `GET /:agentId/activities` (turns with search/dateFrom/dateTo/pagination, returns modelId+source), `GET /:agentId/skills` (builtin allSkills + UserSkill merged), `PUT /:agentId/skills/:id/toggle`, `PUT /:agentId/habits/:jobId/toggle`
      ...
      knowledgeRoutes.ts - /api/knowledge — Agent knowledge base (RAG). POST add (chunk+embed), GET list, GET /search (semantic via embeddingService), DELETE.
    ...
    services/
      ...
      knowledgeService.ts   - Agent knowledge base CRUD + semantic search. Chunking (800 char / 100 overlap), embedding via embeddingService, app-layer cosine similarity (dev mode; prod → pgvector). `addKnowledge` / `listKnowledge` / `searchKnowledge` / `deleteKnowledge`.
      providers/
  ...
  mcp-server/
    ...
    src/skills/
      ...
      skillCreatorSkill.ts - Tier 2 skill wrapping 6 userSkillTools (create/list/update/delete/enable/save_workflow_run_as_skill). Moved from Tier 0 to opt-in. Triggers: 创建技能/create skill/skill creator etc.
      index.ts         - allSkills / skillsByName registries (13 skills: table/idea/taste/analyst/internet-analyst/accounting-analyst/finance-analyst/demo/vibe-design/vibe-coding/worktree/workflow/skill-creator)
frontend/
  src/
...
- **Agent Homepage (Agent Block)**: System block type in MagicCanvas. 7 tabs: Nature (soul/profile/memory in code blocks), Models (builtin+custom cards with current badge), Habits (system+user cron jobs with toggle), Skills (builtin+user with toggle, Skill Creator as Tier 2 skill), Acknowledge (RAG knowledge base, mini file-system UI), Activities (searchable turn log with modelId+source+pagination), Integrations (placeholder). Cross-block sync via `agent-avatar-changed` / `agent-model-changed` / `agent-name-changed` CustomEvents. Agent name written to soul.md `<!-- name: XXX -->` tag on rename. System prompt includes name rules.
...
- **System Habits**: 3 pre-seeded in `ensureAgentFiles()`: `habit_system_evolve` (02:00, update soul/profile), `habit_system_suggest` (08:00, workspace review), `habit_system_learn` (03:00, internet learning, disabled by default). `type: "system"`, can be toggled but not deleted. `evaluateCron` skips `enabled === false` jobs.
