...
- 每个主题 / 来源 / URL 只保存为**一条**完整文档——把所有内容写进一次 \`learn_from_text\` / \`learn_from_url\` 调用的 content 字段里，禁止拆成多次调用。
- content 字段无长度限制，可以放完整的长文。后端会自动做分块和向量化，前端以单个完整文档形式展示给用户。
- 当用户说"学习""保存到知识库"时，先反问用户想学什么内容（URL 或主题），得到回答后再调工具。
