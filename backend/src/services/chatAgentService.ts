...
import { buildLarkResultMessage } from "./integrations/larkResultNormalizer.js";
...
// The summary text starts with a hidden marker `<!-- covered:N -->` where
// N is the number of dropped messages already summarized. Next time we
// only send messages N+1..droppedCount to the LLM (incremental), not the
// full prefix again.

function parseCoveredCount(summary: string | undefined): number {
  if (!summary) return 0;
  const m = summary.match(/^<!-- covered:(\d+) -->\n?/);
  return m ? parseInt(m[1], 10) : 0;
}

function stripCoveredMarker(summary: string): string {
  return summary.replace(/^<!-- covered:\d+ -->\n?/, "");
}

function addCoveredMarker(summary: string, count: number): string {
 ...
 * 2. Upserts a single episodic .md per conversation (serves future conversations)
 *
 * Incremental: only sends newly-dropped messages to the LLM, not messages
 * already covered by the existing summary. Each conversation produces exactly
 * one episodic file (overwritten on update, not appended).
 */
async function generateConversationSummary(
  ...
  try {
    // Check how many messages the existing summary already covers
    const alreadyCovered = parseCoveredCount(existingSummary);
    const totalDropped = droppedMessages.length;

    // Only send the NEW dropped messages (incremental)
    const newMessages = droppedMessages.slice(alreadyCovered);
    if (newMessages.length === 0) {
      // Nothing new to summarize — existing summary already covers everything
      return;
    }
    ...
    const parts: string[] = [];
    const cleanExisting = existingSummary ? stripCoveredMarker(existingSummary) : "";
    if (cleanExisting) {
      parts.push(`[之前的摘要]\n${cleanExisting}\n`);
    }
    parts.push(`[需要补充的新对话（${newMessages.length} 条）]`);
    ...
    // 1. Update conversation summary with coverage marker
    const markedSummary = addCoveredMarker(text.trim(), totalDropped);
    await convStore.updateConversation(conversationId, { summary: markedSummary });
    logAgent({
      event: "conversation_summary_generated",
      conversationId,
      summaryLen: text.trim().length,
      coveredMessages: totalDropped,
      newMessages: newMessages.length,
    });

    // 2. Upsert single episodic file per conversation (overwrite, not append)
    try {
      const { upsertConversationEpisodic, clearWorkingMemory } = await import("./agentService.js");
...
- 当用户问你叫什么、或需要自我介绍时，使用 soul 中的名字回答。
- 名字被修改时，soul.md 会自动同步更新。
...
7. **计划 / 结论必须作为可见文本输出**，不要只留在
   thinking（深度思考）里。用户看不到 thinking 的内容——只有 \`text\` 输出
   才会进主气泡、才会在下一轮被重放给你看。如果你一轮里做了 tool_call 又
   打算等用户选方案，**必须**在这轮结束前用一段文字明确说出来；
   否则用户看到的就是"几个工具卡 + 一片空白"，下一轮你还会忘记自己的计划。

## 执行原则（非常重要）
- **用户意图明确时，直接执行，不要反问**。用户说"帮我建一张表"就建，说"分析数据"就分析，说"写一篇文档"就写。不要回问"你想要几列？""你确定要分析吗？"——先做出来，用户不满意再调整。
- **绝不主动发起文字形式的反问或澄清**，除非用户的请求确实缺少关键信息（例如用户说"删掉那个"但 workspace 有 10 个表,完全无法判断指的是哪个）。即使需要澄清，也优先用最合理的猜测先执行,末尾附一句"如果不是你要的,告诉我调整"。
- **高危操作（删除 / 重置 / 不可撤销的修改）只通过确认卡片（confirm card）征求同意**,不要用文字问"你确定要删除吗？"——系统会自动为带 ⚠️ 标记的工具弹出确认卡片,你直接调用工具即可。
- 总结：你的默认姿态是"先做再说",而不是"先问再做"。用户选择了你就是信任你的判断力。

## 安全红线（不可突破）
- 带 "⚠️" 的删除 / 重置类工具会自动触发确认卡片,你直接调用即可,不需要自己用文字二次确认。
