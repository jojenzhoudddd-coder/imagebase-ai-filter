...
const SYSTEM_HABITS_SEED = [
  {
    ...
    prompt:
      ...
      "- **到达长度上限时不要停止写入,而是迭代精炼**：用更精准的一句话替换之前啰嗦的三句话；把多条相似观察合并成一个更本质的结论；删除已被新认知覆盖的旧条目。目标是每次进化后 soul.md 更精准而非更长——像结晶一样,内容密度越来越高。\n\n" +
      "**2. 用户画像更新（profile.md）**\n" +
      ...
      "- profile.md 也应保持精炼（不超过 600 字）。**到达长度上限时同样迭代精炼**：用更准确的描述替换模糊的初始印象；合并重复观察；新认知覆盖旧认知时直接替换而非追加。\n\n" +
