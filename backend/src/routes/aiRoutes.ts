...
import { generateFilter, generateSort } from "../services/aiService.js";
...
import { FilterGenerateRequest, SortGenerateRequest } from "../types.js";
...
// POST /api/ai/sort/generate  — SSE streaming
router.post("/sort/generate", async (req: Request, res: Response) => {
  const body: SortGenerateRequest = req.body;
  ...
  const userId = (req as any).user?.id ?? null;
  await generateSort(body, fields, sendEvent, {
    userId,
    workspaceId: (table as any).workspaceId ?? null,
  });
