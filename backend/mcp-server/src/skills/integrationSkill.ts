...
原则:
- 默认优先使用官方 provider preset；Lark/飞书使用官方 lark-cli，Figma 优先保存 FIGMA_TOKEN 并使用内置 REST 工具，GitHub 默认使用 gh CLI。
...
- Figma 在托管后端里不能要求用户启动自己电脑上的 127.0.0.1 MCP server；优先使用 FIGMA_TOKEN 的 REST 工具（figma_me / figma_file / figma_file_nodes / figma_images）。只有用户明确提供“后端服务器可访问”的 MCP endpoint 时，才使用 figma_mcp_call。
- 不直接执行用户随口给出的任意命令。需要读取 CLI help 时使用 inspect_cli_help；确认流只用于删除/清空/移除等 delete-like 操作。
