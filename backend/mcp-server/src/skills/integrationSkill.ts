...
- Lark CLI 不要求用户 SSH 到服务器。test_integration 返回 needsAuth 时，调用 start_lark_auth，把 verificationUrl 和 userCode 原样发给用户；用户完成后调用 poll_lark_auth。
- Lark CLI 返回 needsConfig 时，需要先通过 credentials 或服务端环境提供 LARK_APP_ID/LARK_APP_SECRET。
